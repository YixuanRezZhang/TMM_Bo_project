...
class TurboM_bwo(Turbo1):
    ...
    def _select_candidates_each_TR(self, X_cand, y_cand):
        ...
        each_batch_size = min(1, int(self.batch_size/self.n_trust_regions))

        for i in range(self.n_trust_regions):
            X_next = np.zeros((self.batch_size, self.dim))
            idx_next = np.zeros((self.batch_size, 1), dtype=int)
            for k in range(each_batch_size):
    ...
    def _read_log(self):
        ...
        X = data.drop(columns=[self.target_column]).to_numpy()
    ...
    def optimize(self):
        ...
        if self.record_file is None:
            ...
            init = int(os.popen(f'cat {self.record_file} | wc -l').read())
            
        if init > 0:
            if self.suggest_X is None:
                X_old = self._read_log()
            else:
                X_old = self.suggest_X
            maximum_eval = self.max_evals+len(X_old)
        else:
            maximum_eval = self.max_evals
        
        Tstep = 0
        MaxTstep = np.ceil(maximum_eval/(self.n_trust_regions*self.batch_size))
        for i in range(self.n_trust_regions):
            ...
            # if init > 0:
            #     X_init = np.vstack((_X_init, X_old[i::self.n_trust_regions]))
            if init > 0 and i<int(self.n_trust_regions/2):
                X_init = np.vstack((_X_init, X_old[i::int(self.n_trust_regions/2)]))
            else:
                X_init = _X_init
            
            if not self.serial_function:
        ...
        while self.n_evals < maximum_eval:
            ...
            for i in range(self.n_trust_regions):
                ...
                # if fX.min() <= self.fX.min()*1.2:
                if y_cand.min() <= self.fX.min()*1.2:
                    pass
                else:
                    if kk[i] > 0.5:
                        # Exploration phase - swimming
                        r1 = np.random.random_sample((self.n_cand))
                        r2 = np.random.random_sample((self.n_cand))
                        ...
                        if self.dim <= self.n_trust_regions/5:
                            ...
                            X_cands[i][:, params[0]] = X_cand[i][:, params[0]] + ((X_cand[RJ][:, params[0]] - X_cand[i][:, params[1]]) * (r1 + 1) * np.sin(r2 * 2 * math.pi)) *self.length[i]/2
                            X_cands[i][:, params[1]] = X_cand[i][:, params[1]] + ((X_cand[RJ][:, params[0]] - X_cand[i][:, params[1]]) * (r1 + 1) * np.cos(r2 * 2 * math.pi)) *self.length[i]/2
                        else:
                            params = np.arange(self.dim)
                            np.random.shuffle(params)
                            for j in range(round(self.dim/2)):
                                X_cands[i][:, 2*j-1] = X_cand[i][:, params[2*j-1]] + (X_cand[RJ][:, params[0]] - X_cand[i][:, params[2*j-1]]) * (r1 + 1) * np.sin(r2 * 2 * math.pi) *self.length[i]/2
                                X_cands[i][:, 2*j] = X_cand[i][:, params[2*j]] + (X_cand[RJ][:, params[0]] - X_cand[i][:, params[2*j]]) * (r1 + 1) * np.cos(r2 * 2 * math.pi) *self.length[i]/2
                    else:
                        # Development Phase - Predation
                        r3 = np.random.random_sample((self.n_cand))
                        r4 = np.random.random_sample((self.n_cand))
                        ...
                        X_cands[i] = X_cand[i] + (r3 * np.tile(xposbest, (self.n_cand, 1)) - r4 * X_cand[i] + C1 * LevyFlight* (X_cand[RJ] - X_cand[i])) *self.length[i]/2
            ...
            #X_next, idx_next = self._select_candidates_each_TR(X_cands, y_cands)
