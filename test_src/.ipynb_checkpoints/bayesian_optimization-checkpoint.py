...
class MultiFidelityBayesianOptimization:
    def __init__(self, data_file, HF_props, LF_props, feature_props=None, optimization_goal='maximize', scaler_method='standard', 
        ...
        # Data reading and scaling
        _, self.y = self.io_manager.read_data(data_file, target_props=HF_props, feature_props=feature_props)
        self.y = -self.y if self.optimization_goal == 'minimize' else self.y
        ...
        self.X, _ = self.io_manager.read_data(data_file, target_props=[], feature_props=feature_props)
...
class MultiTaskBayesianOptimization:
    ...
    def close_pooling_test(self, n_bootstrap_sample_nums=20, n_iter=100, batch_size=3, hpar=0.1):
        ...
        while current_best >= self.close_pool_threshold:
            XMain_train, XMain_candidate, yMain_train, yMain_candidate = train_test_split(
                self.X, self.y, Mainindices, test_size=1 - self.Main_initial_samples / len(self.X)
                )
            current_best = np.max(np.prod(yMain_train-self.cplb, axis=1))
        print(current_best)
        print(f'Threshold is {self.close_pool_threshold}')
        ...
        for iter in range(n_iter):
            ...
            next_indexes = acquisition_function.select_next(method=self.acq_method, X_candidate=candidate_X_scaled, model_name_list=["correlated_stacking_results_"], num_target=0, model_path=self.model_path+ '_Main', batch_size=batch_size, y_best=current_best)
    ...
    def close_pooling_test0(self, n_bootstrap_sample_nums=20, n_iter=100, batch_size=10, hpar=0.1):
